    ...
    for(auto& object:objects.object_position)
    {
       ...
       if(object.Class == "roomba" && object.probability > 0.99)
       {
           ...
           if((s == 0) || (dist < 7.0 && dist > 0.0))
           {
               ...
               double d_th = 0.5;
               if(roomba_name == 6) d_th = 0.1;
               if(roomba_a_score.score > front_th && roomba_a_score.score-s > d_th && fabs(roomba_a_score.dscore)<0.6)
               {
...
void DistanceBasedLocalizer::calculate_score(double num,double weight,geometry_msgs::PoseStamped &current_pose)
{
   ...
   if(roomba_name == 2) std::cout<<"roomba"<<roomba_name<<"sur"<<sur<<std::endl;
...
{
    double wa = 1.0;
    double x_next = x + 1*cos(yaw);
    double y_next = y + 1*sin(yaw);
